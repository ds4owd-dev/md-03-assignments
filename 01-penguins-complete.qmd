...
5.  What data types are contained in the variables? (Reminder: <https://ds4owd-002.github.io/website/slides/lec-02-visualisation.html#/types-of-variables>)
...
Run the following code chunk and observe the output in the Console. The instructions here would say: Use `filter()` to create a subset from `penguins` that only contains observations for Gentoo penguins.
...
In all the tasks above, you wrote code to transform the `penguins` data. The results appeared in the Console but did not get saved anywhere.

The following code chunk behaves differently. Run it and watch what happens in the Environment pane (top right panel in RStudio).

```{r}
penguins_flipper_sum <- penguins |>
  group_by(species) |>
  summarize(flip_max = max(flipper_length_mm, na.rm = TRUE),
            flip_min = min(flipper_length_mm, na.rm = TRUE))
```
...
2.  What is the difference between this code and the previous code chunks?

3.  Why would we use the assignment operator `<-` to save our results?
...
## Task 6: Commit & Push & Issue

-   Refer back to the instructions on the course website to complete the assignment: <https://ds4owd-002.github.io/website/assignments/md-03/md-03.html>
